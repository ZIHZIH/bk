...
func (articleMongodbDao *ArticleMongodbDao) ArticleInsertOne(ctx context.Context, b string) (string, error) {
	var data bson.D
	err := bson.UnmarshalExtJSON([]byte(b), true, &data)
	if err != nil {
		return "", nil
	}
...
func (articleMongodbDao *ArticleMongodbDao) ArticleUpdateOne(ctx context.Context, jsonStr string) (int64, error) {
	...
	_, err = articleMongodbDao.ArticleFindOne(ctx, updateData["_id"].(string))
	if err != nil {
		return -1, err
	}
...
func NewArticleMongodbDao(ArticleCollection *mongo.Collection, logger *log.Logger) *ArticleMongodbDao {
